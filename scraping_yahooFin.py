...
"""
FILE SOURCE == # Copyright (C) 2015, 2016 Daniel Rodriguez
"""
...
class YahooDownload(object):
    ...
    def __init__(self, ticker, fromdate, todate, period='d', reverse=False):
        ...
        print("=========URL====",url) # FOO_OwnDHANK

        sesskwargs = dict()
        if False and self.p.proxies:
            sesskwargs['proxies'] = self.p.proxies
            print("===DICT==SESSION KWARGS PROXIES ======",sesskwargs) # FOO_OwnDHANK

        crumb = None
        sess = requests.Session()  ## below in FOR Loop --- this requests.Session() --- is being PASSED -PARAMS == url, **sesskwargs
        for i in range(self.retries + 1):  # at least once
            ...
            print("========CRUMB____AAAAAAAAAA===",crumb)
            print("           "*90)
            break
        ...
        print("======urlargs=============",urlargs)
        print("                "*90)


        urld = '{}?{}'.format(urld, '&'.join(urlargs))

        print("==========urld====URLDOWN====BBBBB===",urld)
        print("            "*90)
